...
def update_distribution(x, y, s_hat, categories, Delta_learning, eta_learning, sigma_boundary, no_response, seed):
    ...
    for i in range(n):
        ...
        integral_y = 1 + (eta_learning/(sigma_boundary * C )) * (np.log((np.exp(sigma_boundary * C * (np.max(x) - s_hat[i])) + 1)) - np.log((np.exp(sigma_boundary * C * (np.min(x) - s_hat[i])) + 1)))
        y = y / integral_y
...
def BE_model(func, x, y, s, s_hat, categories, sigma_noise, A_repulsion, eta_learning, sigma_boundary,
    ...
    if mode == 'simulated':
        ...
        ProbB, chooseB, rewards, y = update_distribution(x, y, s_hat, categories, Delta_learning_matrix,
                                                         eta_learning, sigma_boundary, no_response, seed)
        ...
        ProbB, chooseB, rewards, y = update_distribution(x, y, s_hat, categories, Delta_learning_matrix,
                                                         eta_learning, sigma_boundary, no_response, seed)
        data, psychs = func(s[n_burn:], chooseB[n_burn:], rewards[n_burn:], no_response[n_burn:],
                            Not_Blockstart[n_burn:])
