...
from Fitter import total_psychometric, matrix_error, merge_smallest_adjacent, select_and_concatenate, create_random_folds
...
def run_cv_optimization(df, model, func, seed, search_space, k=5, n_trials=50, mode_pre='simulated', fit_with='conditional', sampler_type='TPE'):
    ...
    blocks_in_folds = create_random_folds(sizes, label, k, seed = 4*seed)
    ...
    for fold_idx in range(k):
        ...
        if sampler_type.upper() == 'CMA-ES':
            sampler = optuna.samplers.CmaEsSampler(seed=7*seed)
        elif sampler_type.upper() == 'TPE':
            sampler = optuna.samplers.TPESampler(seed=7*seed)
        else:
