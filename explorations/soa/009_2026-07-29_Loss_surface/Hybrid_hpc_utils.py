...
logging.basicConfig(
    filename="run_pid_cv_errors.log",   # Log file will be created in the current working directory
    filemode="a",                       # <--- MUST BE 'a' (append) FOR PARALLEL CLUSTER JOBS
    level=logging.WARNING,              # Capture WARNING, ERROR, and EXCEPTION messages
    format="%(asctime)s | %(levelname)s | %(module)s:%(lineno)d | %(message)s"
)
logging.warning("🔍 Logging initialized in hpc_utils.py")
# ───────
...
@profile_time
def run_pid_cv(species, p_id, data_path, search_space, seed, cv_folds=5, optuna_trials=200, sampler_type='TPE', save_path=None):
    """
    ...
    # pid_df = pid_df[pid_df['Distribution'].isin(['Asym_right', 'Uniform'])].reset_index(drop=True)
