...
def matrix_error(model_matrix, data_matrix):
    ...
    for i in range(data_matrix.shape[1]):
        data_col = data_matrix[:, i]
        model_col = model_matrix[:, i]

        valid_mask = ~np.isnan(data_col) & ~np.isnan(model_col)
        if not np.any(valid_mask):
            continue

        v = data_col[valid_mask]
        w = model_col[valid_mask]
    ...
    return total_error / valid_cols if valid_cols > 0 else np.nan
...
def create_random_folds(blocks, labels=None, num_folds=5, seed=None):
    """
    Randomly assigns blocks to a specified number of folds, 
    ensuring an equal (or near-equal) number of blocks per fold.
    
    Parameters:
    blocks (list): The list of data blocks.
    labels (list): Optional labels or IDs for the blocks.
    num_folds (int): The number of folds to split the data into.
    seed (int): Optional random seed for reproducibility.
    """
    # Create a local random generator. If no seed is provided, it acts truly randomly.
    rng = random.Random(seed) if seed is not None else random
    
    # If no labels are provided, default to using the indices of the blocks
    if labels is None:
        labels = list(range(len(blocks)))
    else:
    ...
    # 1. Randomize the order of the blocks using the seeded generator
    rng.shuffle(labels)

    # 2. Initialize empty lists for each fold
    folds = [[] for _ in range(num_folds)]

    # 3. Distribute the randomized blocks evenly across the folds
    for i, label in enumerate(labels):
        folds[i % num_folds].append(label)
...
def post_correct_binned_counts(s, chooseB, reward, No_response, Not_Blockstart, n_bins=8):
    s = np.asarray(s, dtype=float)
    chooseB = np.asarray(chooseB, dtype=float)
    reward = np.asarray(reward, dtype=float)
    No_response = np.asarray(No_response).astype(bool)
    Not_Blockstart = np.asarray(Not_Blockstart).astype(bool)

    edges = np.linspace(-1, 1, n_bins + 1)

    current_bins = np.digitize(s[1:], edges) - 1
    previous_bins = np.digitize(s[:-1], edges) - 1

    current_bins = np.clip(current_bins, 0, n_bins - 1)
    previous_bins = np.clip(previous_bins, 0, n_bins - 1)
    ...
    N_cond = np.zeros((n_bins, n_bins), dtype=float)
    B_cond = np.zeros((n_bins, n_bins), dtype=float)

    N_total = np.zeros(n_bins, dtype=float)
    B_total = np.zeros(n_bins, dtype=float)

    for i_cur, j_prev, ch in zip(current_bins[valid], previous_bins[valid], chooseB[1:][valid]):
        N_cond[i_cur, j_prev] += 1.0
        B_cond[i_cur, j_prev] += ch

        N_total[i_cur] += 1.0
        B_total[i_cur] += ch

    return N_cond, B_cond, N_total, B_total


def matrices_from_binned_counts(N_cond, B_cond, N_total, B_total):
    with np.errstate(divide="ignore", invalid="ignore"):
        conditional = B_cond / N_cond
        total_psych = B_total / N_total

    conditional[N_cond == 0] = np.nan
    total_psych[N_total == 0] = np.nan

    update = conditional - total_psych[:, None]

    return update, conditional, total_psych


def post_correct_update_matrix_binned(s, chooseB, reward, No_response, Not_Blockstart, n_bins=8):
    N_cond, B_cond, N_total, B_total = post_correct_binned_counts(
        s=s,
        chooseB=chooseB,
        reward=reward,
    ...
    update, conditional, total_psych = matrices_from_binned_counts(
        N_cond=N_cond,
        B_cond=B_cond,
        N_total=N_total,
        B_total=B_total
