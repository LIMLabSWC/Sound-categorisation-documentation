...
def Hybrid_model(func, x, s, s_hat, categories, sigma_noise, A_repulsion, gamma, sigma_update, y_BE, Delta_learning, eta_learning, sigma_boundary, alpha,lambda_A, lambda_B, no_response,
    ...
    if mode == 'simulated':
        ...
        Final_data, Final_psychs = func(s, Final_Response[m:], Final_rewards[m:], no_response[m:], Not_Blockstart)
        data_SC, psychs_SC = func(s, Response_SC[m:], rewards_SC[m:], no_response[m:], Not_Blockstart)
        data_BE, psychs_BE = func(s, Response_BE[m:], rewards_BE[m:], no_response[m:], Not_Blockstart)
        ...
        # Exclude simulated trials from analysis
        Final_data, Final_psychs = func(s[n_burn:], Final_Response[n_burn:], Final_rewards[n_burn:], no_response[n_burn:], Not_Blockstart[n_burn:])
        data_SC, psychs_SC = func(s[n_burn:], Response_SC[n_burn:], rewards_SC[n_burn:], no_response[n_burn:], Not_Blockstart[n_burn:])
        data_BE, psychs_BE = func(s[n_burn:], Response_BE[n_burn:], rewards_BE[n_burn:], no_response[n_burn:], Not_Blockstart[n_burn:])

    else:
        raise ValueError("Invalid mode. Use 'simulated' or 'real'.")
    

    Final_update_matrix = Final_data[::-1]
    Final_conditional_matrix = Final_psychs[::-1]

    SC_update_matrix = data_SC[::-1]
    SC_conditional_matrix = psychs_SC[::-1]

    BE_update_matrix = data_BE[::-1]
    BE_conditional_matrix = psychs_BE[::-1]

    return Final_update_matrix, Final_conditional_matrix, SC_update_matrix, SC_conditional_matrix, BE_update_matrix, BE_conditional_matrix




def Hybrid_model_outputs(
    x, s, s_hat, categories,
    sigma_noise, A_repulsion,
    gamma, sigma_update,
    y_BE, Delta_learning,
    eta_learning, sigma_boundary,
    ...
    if mode == "simulated":
        ...
        s_hat_all = np.concatenate((s_hat_pre, s_hat))
        categories_all = np.concatenate((categories_pre, categories))
        no_response_all = np.concatenate((no_response_pre, no_response))

        Delta_learning_matrix = Delta_learning(x, s_hat_all, categories_all, sigma_boundary)
        ...
        (
            ...
            y_BE=y_BE,
            Delta_learning=Delta_learning_matrix,
            eta_learning=eta_learning,
            alpha=alpha,
            lambda_A=lambda_A,
            lambda_B=lambda_B,
            no_response=no_response_all,
            seed=seed
        )
        ...
        return {
            ...
            "Final_choice": np.asarray(Final_Response[m:]),
            "Final_reward": np.asarray(Final_rewards[m:]),
            "SC_choice": np.asarray(Response_SC[m:]),
            "SC_reward": np.asarray(rewards_SC[m:]),
            "BE_choice": np.asarray(Response_BE[m:]),
            "BE_reward": np.asarray(rewards_BE[m:]),
        }
        ...
        return {
            "s": np.asarray(s[n_burn:]),
            "no_response": np.asarray(no_response[n_burn:]),
            "Not_Blockstart": np.asarray(Not_Blockstart[n_burn:]),
            "Final_choice": np.asarray(Final_Response[n_burn:]),
            "Final_reward": np.asarray(Final_rewards[n_burn:]),
            "SC_choice": np.asarray(Response_SC[n_burn:]),
            "SC_reward": np.asarray(rewards_SC[n_burn:]),
            "BE_choice": np.asarray(Response_BE[n_burn:]),
            "BE_reward": np.asarray(rewards_BE[n_burn:]),
        }
