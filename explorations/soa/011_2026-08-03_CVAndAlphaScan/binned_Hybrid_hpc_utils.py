...
from binned_Gradient_free_Opt import run_repeated_cv_optimization, run_one_repetition_optimization, run_full_dataset_optimization
...
def get_variant_specs(model_mode="all", custom_alpha=None, alpha_scan=False):
    hybrid_space = {
    ...
    fixed_alpha_space = be_space | sc_space
    ...
    if model_mode == "fixed_alpha":
        if custom_alpha is None and not alpha_scan:
            raise ValueError("custom_alpha must be provided when model_mode='fixed_alpha'")

        if custom_alpha is not None and not (0.0 <= custom_alpha <= 1.0):
            raise ValueError("custom_alpha must be between 0 and 1")

        return {
            "Hybrid_fixed_alpha": {
                "search_space": fixed_alpha_space,
                "fixed_alpha": None if alpha_scan else float(custom_alpha),
            }
        }
...
def run_pid_repeated_cv(
    ...
    alpha_scan=(fit_mode == "full_alpha_scan"),
)
    ...
    print(f"  ✱ Model seeds: {model_seed_count}")
    ...
    if fit_mode == "cv":
        ...
        for variant_name, spec in variants.items():
            for r_idx, partition_seed in enumerate(repetition_seeds, start=1):
                ...
                task_keys.append((variant_name, r_idx))
        ...
        for (variant_name, r_idx), (partition, fits) in zip(task_keys, outputs):
            ...
            grouped[variant_name]["fits"].extend(fits)

        for variant_name, variant_data in grouped.items():
            train_errors = np.array(
                [x["train_error"] for x in variant_data["fits"]],
                dtype=float
            )
            test_errors = np.array(
                [x["test_error"] for x in variant_data["fits"]],
                dtype=float
            )
        ...
        for variant_name, spec in variants.items():
            print("\n==============================")
            print(f"Running FULL variant: {variant_name}")
            print("==============================")

            full_result = run_full_dataset_optimization(
                df=pid_df,
                model_outputs=Hybrid_model_outputs,
                search_space=spec["search_space"],
                n_trials=optuna_trials,
            ...
            results["variants"][variant_name] = full_result

    elif fit_mode == "full_alpha_scan":
        fixed_alpha_space = variants["Hybrid_fixed_alpha"]["search_space"]

        tasks = [
            delayed(run_full_dataset_optimization)(
                df=pid_df,
                model_outputs=Hybrid_model_outputs,
                search_space=fixed_alpha_space,
                n_trials=optuna_trials,
        ...
        for alpha, out in zip(alpha_values, outputs):
            key = f"Hybrid_fixed_alpha_{alpha:.1f}".replace(".", "p")
            results["variants"][key] = out

    else:
        raise ValueError("fit_mode must be one of: cv, full, full_alpha_scan")

    save_results(results, save_path)

    print(f"  ✔ Completed {fit_mode} fitting for {species} subject {p_id}")
    return results
...
def build_save_path(
    ...
    if fit_mode == "full_alpha_scan":
        model_folder = "fixed_alpha_scan"
    elif model_mode == "fixed_alpha":
    ...
    save_name = f"results_{species}_{p_id}_{sampler_type}_{fit_mode}_{model_folder}.pkl"
